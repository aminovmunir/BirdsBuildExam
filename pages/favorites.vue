<template>
  <div class="container">
    ...
    <section class="content">
      ...
      @addDeal="addToDeal"
      v-for="product in store.favorites" :key="'product-' + product.id"
...
onMounted(async ()=>{
  ...
  await store.getFavorites(fingerprint)
...
const filter = async (selling_type: string) => {
  ...
  store.setLoading(true)
  await store.getFavorites(fingerprint, selling_type, name)
  store.setLoading(false)
}

const search = (name: string): void => {
  const selling_type: string | null = localStorage.getItem('selling_type')
  store.getFavorites(fingerprint, selling_type, name)
}
